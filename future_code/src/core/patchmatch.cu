...
bool PatchMatch::loadData()
{
  ...
  for (int i = 0; i < images_h.size(); i++) {
    ...
    if (std::max(images_h[i].width, images_h[i].height) > opt_h->cur_size) {
      images_h[i].rescale(opt_h->cur_size);
...
bool PatchMatch::setCurrentResult()
{
  ...
  if (opt_h->upsample) {
    depth_cur.rescale(opt_h->cur_size);
    cost_cur.rescale(opt_h->cur_size);
    normal_cur.rescale(opt_h->cur_size);
  }
...
bool PatchMatch::setPlanarPriors()
{
  Delaunay delaunay(frame_h, opt_h);
  if (!delaunay.computePlanarPriors()) {
    std::cout << "Failed to compute planar priors!" << std::endl;
    return false;
  }
  ...
  for (int col = 0; col < width; col++) {
    for (int row = 0; row < height; row++) {
      const int idx = row * width + col;
      const float mask = delaunay.getMask(row, col);
      prior_masks_h[idx] = static_cast<unsigned int>(mask);
      if (mask > 0) {
        prior_planes_h[idx] = delaunay.getPlane(mask - 1);
      }
    }
  }
...
bool PatchMatch::moveDataToCuda(cuda::device_t device)
{
  ...
  if (opt_h->planar_priors) {
    prior_masks_h = std::vector<unsigned int>(num_pixels);
    prior_masks_d = cuda::memory::device::make_unique<unsigned int[]>(device, num_pixels);

    prior_planes_h = std::vector<Plane>(num_pixels);
    prior_planes_d = cuda::memory::device::allocate(device, num_pixels * sizeof(Plane));

    if (!setPlanarPriors()) {
      std::cout << "Failed to set planar priors!" << std::endl;
      return false;
    }

    cuda::memory::copy(prior_masks_d.get(), prior_masks_h.data(), num_pixels * sizeof(unsigned int));
    cuda::memory::copy(prior_planes_d.get(), prior_planes_h.data(), num_pixels * sizeof(Plane));
  }
  ...
  if (opt_h->geom_cons || opt_h->planar_priors || opt_h->upsample) {
    if (!setCurrentResult()) {
  ...
  if (opt_h->upsample) {
    pre_costs_d = cuda::memory::device::make_unique<float[]>(device, num_pixels);
  }
...
bool PatchMatch::run()
{
  ...
  auto prior_planes_ptr = static_cast<Plane*>(prior_planes_d.get());
  ...
  else if (opt_h->planar_priors) {
    num_iterations = opt_h->num_planar_iterations;
  }
...
PatchMatch::~PatchMatch()
{
  ...
  if (opt_h->planar_priors) {
    cuda::memory::device::free(prior_planes_d);
  }
}