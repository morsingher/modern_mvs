...
CUDA_FD int findMaxCostIndex(const float* costs, const int n)
{
...
CUDA_FD float getAngleDifference(const Plane& p1, const Plane& p2)
{
  const Eigen::Vector3d n1 = p1.normal.normalized();
  const Eigen::Vector3d n2 = p2.normal.normalized();
  return acos(n1.dot(n2));
}

// Equation 6 in ACMP paper

CUDA_FD float computePrior(const Plane& p_cur, const Plane& prior, const Camera& cam, const Pixel pix)
{
  const float gamma = 0.5;
  const float depth_sigma = (cam.max_depth - cam.min_depth) / 64.0;
  const float depth_var = 2 * depth_sigma * depth_sigma;
  const float angle_sigma = M_PI * (5.0 / 180.0);
  const float angle_var = 2 * angle_sigma * angle_sigma;

  const float depth_prior = getDepthFromPlane(cam, prior, pix);
  const float depth_cur = getDepthFromPlane(cam, p_cur, pix);
  const float depth_diff = depth_cur - depth_prior;
  const float angle_diff = getAngleDifference(p_cur, prior);

  return gamma * expf(-(depth_diff * depth_diff) / depth_var) * expf(-(angle_diff * angle_diff) / angle_var);
}
    ...
    const Options* opt)
{
  ...
  // Random plane (if a prior exist, perturb that)

  Eigen::Vector3d n_rand;
  float d_rand, depth_prior;
  if (opt->planar_priors && prior_masks[idx] > 0) {
    depth_prior = getDepthFromPlane(cameras[0], prior_planes[idx], pix);
    const float depth_sigma = (cameras[0].max_depth - cameras[0].min_depth) / 64.0;
    d_rand = curand_uniform(rand) * 6 * depth_sigma + (depth_prior - 3 * depth_sigma);
    n_rand = perturbNormal(cameras[0], pix, prior_planes[idx], rand, perturbation * M_PI);
  }
  else {
    Plane p_rand;
    p_rand.setRandom(cameras[0], pix, rand);
    n_rand = p_rand.normal;
    d_rand = getDepthFromPlane(cameras[0], p_rand, pix);
  }
  ...
  for (int i = 0; i < num_planes; ++i) {
    ...
    if (opt->planar_priors && prior_masks[idx] > 0) {
      ...
      const float prior = computePrior(new_plane, prior_planes[idx], cameras[0], pix);
      const float cost_new_with_prior = expf(-(temp_cost * temp_cost) / beta) * prior;
      if (new_depth >= cameras[0].min_depth && new_depth <= cameras[0].max_depth &&
          cost_new_with_prior < *cost_with_prior) {
        *depth = new_depth;
        *plane = new_plane;
        *cost = temp_cost;
        *cost_with_prior = cost_new_with_prior;
      }
    }
    else if (new_depth >= cameras[0].min_depth && new_depth <= cameras[0].max_depth && temp_cost < *cost) {
      *depth = new_depth;
      *plane = new_plane;
      *cost = temp_cost;
    }
    ...
    const int iter)
{
  ...
  // Include planar priors

  float cost_with_prior = 0.0;
  Plane plane_cur = planes[idx];
  if (opt->planar_priors && prior_masks[idx] > 0) {
    float cost_with_prior_vec[8] = {0.0};
    const float beta = 0.18;
    for (int i = 0; i < 8; i++) {
      if (flag[i]) {
        const float prior = computePrior(planes[positions[i]], prior_planes[idx], cameras[0], pix);
        cost_with_prior_vec[i] = exp(-final_costs[i] * final_costs[i] / beta) * prior;  // eq 4 ACMP paper
      }
    }
    const int max_cost_idx = findMaxCostIndex(cost_with_prior_vec, 8);  // Max of negative is min of positive

    if (flag[max_cost_idx]) {
      const float prior_cur = computePrior(plane_cur, prior_planes[idx], cameras[0], pix);
      const float cost_new_with_prior = exp(-cost_cur * cost_cur / beta) * prior_cur;
      const float depth_new = getDepthFromPlane(cameras[0], planes[positions[max_cost_idx]], pix);
      if (depth_new >= cameras[0].min_depth && depth_new <= cameras[0].max_depth &&
          cost_with_prior_vec[max_cost_idx] > cost_new_with_prior) {
        depth_cur = depth_new;
        plane_cur = planes[positions[max_cost_idx]];
        cost_cur = final_costs[max_cost_idx];
        cost_with_prior = cost_with_prior_vec[max_cost_idx];
    ...
    if (depth_new >= cameras[0].min_depth && depth_new <= cameras[0].max_depth && cost_new < cost_cur) {
      depth_cur = depth_new;
      plane_cur = planes[positions[min_cost_idx]];
      cost_cur = cost_new;
      views[idx] = temp_views;
    }
  }
  ...
  if (!opt->upsample || (opt->upsample && cost_cur < pre_costs[idx] - 0.1)) {
    costs[idx] = cost_cur;
    planes[idx] = plane_cur;
  }
}
    ...
    float* pre_costs)
{
  ...
  // Three cases:
  // 1) Geometric/upsampling -> rotate normal to cam and convert depth to dist
  // 2) Planar -> perturb the current prior where it exists
  // 3) Photometric -> set to random

  if (opt->planar_priors) {
    if (prior_masks[idx] > 0 && costs[idx] > 0.1) {
      const float perturbation = 0.06;
      float d_pert = prior_planes[idx].dist;
      const float min_d_pert = (1.0 - perturbation) * d_pert;
      const float max_d_pert = (1.0 + perturbation) * d_pert;
      d_pert = curand_uniform(&rand[idx]) * (max_d_pert - min_d_pert) + min_d_pert;
      const Eigen::Vector3d n_pert = perturbNormal(cameras[0], pix, prior_planes[idx], &rand[idx], perturbation * M_PI);
      planes[idx] = Plane(n_pert, d_pert);
    }
    else {
  ...
  else if (opt->geom_cons || opt->upsample) {
    planes[idx].rotateNormalToCamera(cameras[0]);
    planes[idx].setDistanceFromDepth(cameras[0], pix, planes[idx].dist);
  }
  else {
