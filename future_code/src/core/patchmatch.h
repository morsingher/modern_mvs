...
class PatchMatch {
 public:
  ...
  std::vector<Plane> prior_planes_h;
  cuda::memory::region_t prior_planes_d;

  std::vector<unsigned int> prior_masks_h;
  cuda::memory::device::unique_ptr<unsigned int[]> prior_masks_d;
  ...
  cuda::memory::device::unique_ptr<float[]> pre_costs_d;
};
